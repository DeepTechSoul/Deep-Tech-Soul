# Hi, I'm Sonali Moon 👋

**Data Scientist | Machine Learning | Python | Power BI | Streamlit**

I'm a data scientist with a background in Industrial Engineering (B.E.) and Industrial Hygiene (M.S., West Virginia University, USA). I recently completed a 6-month Data Science internship at Spinnaker Analytics LLC where I built and deployed end-to-end machine learning projects across regression, classification, and clustering.

I'm passionate about turning messy real-world data into clear, actionable insights — and making those insights accessible through interactive dashboards and web apps.

---

## 🗂️ Projects

| Project | Type | Tools |
|---|---|---|
| [Used Car Price Prediction](https://github.com/DeepTechSoul/used-car-price-prediction) | Regression | Python, XGBoost, Streamlit |
| [Hotel Reservation Cancellation](https://github.com/DeepTechSoul/Hotel-Cancellation) | Classification | Python, Random Forest, Streamlit |
| [Rainfall Prediction — Mumbai](https://github.com/DeepTechSoul/Rainfall-Prediction) | Regression | Python, XGBoost, Streamlit |
| [Shooting Incident Fatality](https://github.com/DeepTechSoul/Shooting-Incident-Fatality) | Classification | Python, Scikit-learn, Streamlit |
| [Product Cluster Analysis](https://github.com/DeepTechSoul/Product-Cluster-Analysis) | Clustering | Python, K-Means, Power BI |
| [Appliance Energy Consumption](https://github.com/DeepTechSoul/Appliance-Energy-Consumption) | Regression | Python, Gradient Boosting, Scikit-learn |
## 🛠️ Skills

**Languages & Tools:** Python, SQL, Power BI, Tableau, Streamlit  
**Libraries:** Pandas, NumPy, Scikit-learn, XGBoost, TensorFlow, Keras, Matplotlib, Seaborn  
**ML Techniques:** Regression, Classification, Clustering, NLP, Deep Learning  
**Other:** Generative AI, OpenAI API, DALL-E, Stable Diffusion

---

## 📫 Connect with Me
- 💼 [LinkedIn](https://www.linkedin.com/in/sonali-v-13b05713)
- 📧 shonali24@gmail.com

---
*Open to remote Data Scientist / Data Analyst opportunities*
